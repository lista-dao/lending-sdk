...
vi.mock('../read/market/getMarketExtraInfo', () => ({
    getMarketExtraInfo: vi.fn().mockResolvedValue({
        ...
        collateralInfo: { address: '0x0000000000000000000000000000000000000002', decimals: 18, symbol: 'COLL' },
        loanInfo: { address: '0x0000000000000000000000000000000000000001', decimals: 18, symbol: 'LOAN' },
...
vi.mock('../read/vault/getVaultInfo', () => ({
    getVaultInfo: vi.fn().mockResolvedValue({
        ...
        assetInfo: { address: '0x2222222222222222222222222222222222222222', decimals: 18, symbol: 'ASSET' },
        isNative: false,
...
vi.mock('../read/smart/getSmartMarketExtraInfo', () => ({
    getSmartMarketExtraInfo: vi.fn().mockResolvedValue({
        ...
        lpInfo: { address: '0x7777777777777777777777777777777777777777', decimals: 18, symbol: 'LP' },
        loanInfo: { address: '0x0000000000000000000000000000000000000001', decimals: 18, symbol: 'LOAN' },
        tokenAInfo: { address: '0x8888888888888888888888888888888888888888', decimals: 18, symbol: 'TOKA' },
        tokenBInfo: { address: '0x9999999999999999999999999999999999999999', decimals: 18, symbol: 'TOKB' },
    }),
}));
...
vi.mock('../read/broker/getBrokerFixedTerms', () => ({
    getBrokerFixedTerms: vi.fn().mockResolvedValue([
...
vi.mock('@lista-dao/moolah-sdk-core', async (importOriginal) => {
    const actual = await importOriginal<typeof import('@lista-dao/moolah-sdk-core')>();
    return {
        ...actual,
        MoolahApiClient: vi.fn().mockImplementation(() => ({
            getMarketInfo: vi.fn().mockResolvedValue({ id: 'market1', name: 'Test Market' }),
            getVaultList: vi.fn().mockResolvedValue({ list: [], total: 0 }),
            getVaultInfo: vi.fn().mockResolvedValue({ address: '0x1111111111111111111111111111111111111111' }),
...
describe('MoolahSDK', () => {
    ...
    describe('read methods', () => {
        ...
        it('should get broker user positions', async () => {
            const result = await sdk.getBrokerUserPositions(56, BROKER_ADDRESS, WALLET);
            expect(result).toBeDefined();
        });
    });
    ...
    describe('API methods', () => {
        ...
        it('should get vault list from API', async () => {
            const result = await sdk.getVaultList({ chain: 'bsc', page: 1, pageSize: 20 });
            expect(result).toBeDefined();
        ...
        it('should get market list from API', async () => {
            const result = await sdk.getMarketList({ chain: 'bsc', page: 1, pageSize: 20 });
            expect(result).toBeDefined();
            expect(result.list).toBeDefined();
        });
    ...
    describe('build smart market methods', () => {
        ...
        it('should build smart withdraw collateral params', async () => {
            ...
            expect(result).toBeDefined();
            expect(result.some((s) => s.step === 'withdrawSmartCollateral')).toBe(true);
        });
        ...
        it('should build smart withdraw collateral fixed params', async () => {
            ...
            expect(result).toBeDefined();
            expect(result.some((s) => s.step === 'withdrawSmartCollateralFixed')).toBe(true);
        });
    ...
    describe('chain handling', () => {
        ...
        it('should throw for missing RPC URL', async () => {
            ...
            await expect(sdkNoRpc.buildBorrowParams({
                chainId: 56,
                marketId: MARKET_ID,
                assets: 500n,
                walletAddress: WALLET,
