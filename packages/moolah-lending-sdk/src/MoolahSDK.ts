...
export class MoolahSDK {
    ...
    async getMarketExtraInfo(chainId: ChainId, marketId: Address): Promise<MarketExtraInfo> {
        const publicClient = this.getPublicClient(chainId);
        const contracts = this.getContracts(chainId);
        const network = this.getNetwork(chainId);
        return getMarketExtraInfo(publicClient, contracts as any, network, marketId);
    }
    ...
    async getMarketUserData(
        ...
        const publicClient = this.getPublicClient(chainId);
        const contracts = this.getContracts(chainId);
        const extraInfo = marketExtraInfo ?? (await this.getMarketExtraInfo(chainId, marketId));
        return getMarketUserData(publicClient, contracts as any, marketId, userAddress, extraInfo, fixedTermData);
    }

    async getWriteConfig(chainId: ChainId, marketId: Address): Promise<WriteMarketConfig> {
        const extraInfo = await this.getMarketExtraInfo(chainId, marketId);
        return toWriteConfig(extraInfo);
    }

    async getVaultInfo(chainId: ChainId, vaultAddress: Address): Promise<VaultInfo> {
        const publicClient = this.getPublicClient(chainId);
        const contracts = this.getContracts(chainId);
        const network = this.getNetwork(chainId);
        return getVaultInfo(publicClient, contracts as any, network, vaultAddress);
    }
    ...
    async getSmartMarketExtraInfo(chainId: ChainId, marketId: Address): Promise<SmartMarketExtraInfo> {
        const publicClient = this.getPublicClient(chainId);
        const contracts = this.getContracts(chainId);
        const network = this.getNetwork(chainId);
        return getSmartMarketExtraInfo(publicClient, contracts as any, network, marketId);
    }
    ...
    async getSmartMarketUserData(
        ...
        const publicClient = this.getPublicClient(chainId);
        const contracts = this.getContracts(chainId);
        const info = extraInfo ?? (await this.getSmartMarketExtraInfo(chainId, marketId));
        return getSmartMarketUserData(publicClient, contracts as any, marketId, userAddress, info);
    }

    async getBrokerFixedTerms(chainId: ChainId, brokerAddress: Address): Promise<FixedTermAndRate[]> {
        const publicClient = this.getPublicClient(chainId);
    ...
    async getBrokerUserPositions(
        ...
        const publicClient = this.getPublicClient(chainId);
        const network = this.getNetwork(chainId);
        const rateCalculatorAddress = getContractAddressOptional(network, 'brokerRateCalculator');
        return getBrokerUserPositions(publicClient, brokerAddress, rateCalculatorAddress, userAddress, loanDecimals);
    }

    // ===== Read Methods (API) =====

    async getMarketInfo(chainId: ChainId, marketId: Address): Promise<MarketInfo> {
        return this.apiClient.getMarketInfo(marketId);
    }

    async getVaultList(params: ApiTableParams & {
        assets?: string[];
        curators?: string[];
        chain: string;
    ...
    async getMarketList(params: ApiTableParams & {
        loans?: string[];
        collaterals?: string[];
        termType?: number;
        chain: string;
    ...
    async buildSupplyParams(params: BuildSupplyParams): Promise<StepParam[]> {
        ...
        const marketInfo = params.marketInfo ?? (await this.getWriteConfig(params.chainId, params.marketId));
    ...
    async buildRepayParams(params: BuildRepayParams): Promise<StepParam[]> {
        ...
        if (params.repayAll && !userData) {
            userData = await this.getMarketUserData(params.chainId, params.marketId, params.walletAddress);
        }
    ...
    async buildWithdrawParams(params: BuildWithdrawParams): Promise<StepParam[]> {
        ...
        if (params.withdrawAll) {
            const userData = params.userData ?? (await this.getMarketUserData(params.chainId, params.marketId, params.walletAddress));
    ...
    async buildVaultDepositParams(params: BuildVaultDepositParams): Promise<StepParam[]> {
        const network = this.getNetwork(params.chainId);
        const publicClient = this.getPublicClient(params.chainId);
        const vaultInfo = params.vaultInfo ?? (await this.getVaultInfo(params.chainId, params.vaultAddress));
    ...
    async buildVaultWithdrawParams(params: BuildVaultWithdrawParams): Promise<StepParam[]> {
        const network = this.getNetwork(params.chainId);
        const vaultInfo = params.vaultInfo ?? (await this.getVaultInfo(params.chainId, params.vaultAddress));

        let userData = params.userData;
        if (params.withdrawAll && !userData) {
            userData = await this.getVaultUserData(params.chainId, params.vaultAddress, params.walletAddress, vaultInfo);
        }
    ...
    async buildSmartSupplyDexLpParams(params: BuildSmartSupplyDexLpParams): Promise<StepParam[]> {
        const network = this.getNetwork(params.chainId);
        const publicClient = this.getPublicClient(params.chainId);
        const smartConfig = params.smartConfig ?? (await this.getSmartMarketExtraInfo(params.chainId, params.marketId));
    ...
    async buildSmartSupplyCollateralParams(params: BuildSmartSupplyCollateralParams): Promise<StepParam[]> {
        const network = this.getNetwork(params.chainId);
        const publicClient = this.getPublicClient(params.chainId);
        const smartConfig = params.smartConfig ?? (await this.getSmartMarketExtraInfo(params.chainId, params.marketId));
    ...
    async buildSmartWithdrawDexLpParams(params: BuildSmartWithdrawDexLpParams): Promise<StepParam[]> {
        const smartConfig = params.smartConfig ?? (await this.getSmartMarketExtraInfo(params.chainId, params.marketId));
    ...
    async buildSmartWithdrawCollateralParams(params: BuildSmartWithdrawCollateralParams): Promise<StepParam[]> {
        const smartConfig = params.smartConfig ?? (await this.getSmartMarketExtraInfo(params.chainId, params.marketId));
    ...
    async buildSmartWithdrawCollateralFixedParams(params: BuildSmartWithdrawCollateralFixedParams): Promise<StepParam[]> {
        const smartConfig = params.smartConfig ?? (await this.getSmartMarketExtraInfo(params.chainId, params.marketId));
    ...
    async buildSmartRepayParams(params: BuildSmartRepayParams): Promise<StepParam[]> {
        const network = this.getNetwork(params.chainId);
        const publicClient = this.getPublicClient(params.chainId);
        const smartConfig = params.smartConfig ?? (await this.getSmartMarketExtraInfo(params.chainId, params.marketId));
        ...
        let userData = params.userData;
        if (params.repayAll && !userData) {
            userData = await this.getSmartMarketUserData(params.chainId, params.marketId, params.walletAddress);
        }
    ...
    async buildBrokerBorrowParams(params: BuildBrokerBorrowParams): Promise<StepParam[]> {
    ...
    async buildBrokerRepayParams(params: BuildBrokerRepayParams): Promise<StepParam[]> {
        const network = this.getNetwork(params.chainId);
        const publicClient = this.getPublicClient(params.chainId);
