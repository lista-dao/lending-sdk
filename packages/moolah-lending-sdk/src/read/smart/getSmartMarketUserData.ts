...
export async function getSmartMarketUserData(
    ...
    const [lpBalance, loanBalance, tokenABalance, tokenBBalance] = await Promise.all([
    ...
    const baseBorrowed =
        extraInfo.totalBorrowShares > 0n
            ? new Decimal(
                (borrowShares * extraInfo.totalBorrowAssets) / extraInfo.totalBorrowShares,
                loanInfo.decimals,
            )
            : Decimal.ZERO;
    ...
    const _computeWithdrawable = (
        ...
        if (isFixed) {
            const maxWithdrawA = lpBalances[0]
                .mul(maxWithdrawLp)
                .div(totalLp);
            const maxWithdrawB = lpBalances[1]
                .mul(maxWithdrawLp)
                .div(totalLp);
