...
import { getERC20Balance, getUserPosition, isWhiteList } from '@lista-dao/moolah-sdk-core';
...
export async function getMarketUserData(
    ...
    const finalBorrowed = fixedTermData && !fixedTermData.totalBorrowed.isZero()
        ? fixedTermData.totalBorrowed
        : borrowed;

    const finalBorrowRate = fixedTermData && !fixedTermData.weightedBorrowRate.isZero()
    ...
    const _getExtraRepayAmount = () => {
        ...
        const unrealizedRate = new Decimal(
            MathLib.wTaylorCompounded(
                rateView,
                now - currentTime + BigInt(20 * 60),
            ),
        );
    ...
    const _computeLoanable = (
        supplyAmount: string | number,
        LTV?: number,
    ) => {
        const supply = Decimal.parse(supplyAmount ?? 0, decimals.c);
        const currentLTV =
            LTV == null ? LLTV.mul(THRESHOLD) : Decimal.parse(LTV);
